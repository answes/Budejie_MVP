...
import com.bigshark.budejie_mvp.utils.DateUtils;
...
//        ImageLoader.ImageListener listener = ImageLoader.getImageListener(holder.iv_video,
//                R.drawable.essence_cai_normal, R.drawable.essence_cai_normal);//参数分别为要显示的图片控件，默认显示的图片
        if (!TextUtils.isEmpty(postList.getGifFistFrame())) {
//            holder.iv_video.setImageUrl(postList.getBimageuri(), imageLoader);

            loadImage(holder.iv_video, postList.getGifFistFrame());
        }
    }
    ...
    class VideoAdapterViewHolder extends RecyclerView.ViewHolder {
        ...
        public NetworkImageView iv_video;
        ...
        public VideoAdapterViewHolder(View itemView, boolean isItem) {
            ...
            if (isItem) {
                ...
                iv_video = (NetworkImageView) itemView
